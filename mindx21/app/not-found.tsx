...
export default function NotFound() {
  return (
    <div className="mx-auto max-w-wide px-5 py-24 sm:px-8">
      ...
      <p className="reading mt-5 max-w-md">
        The link may be old. Everything on the site is one click from here.
      </p>

      <div className="mt-9 flex flex-wrap gap-x-8 gap-y-2 font-serif text-[0.95rem]">
      ...
      <div className="mt-14 max-w-lg border-t border-line/[.16] pt-6">
        <p className="mb-3 type-dek">Or start in a region:</p>
        <ul className="flex flex-wrap gap-x-6 gap-y-2">
          {regions.map((r) => (
            <li key={r.slug}>
              <Link
                href={`/regions/${r.slug}`}
                className="inline-flex min-h-11 items-center type-label underline-offset-4 transition-colors hover:text-accent hover:underline"
              >
                {r.short}
              </Link>
            </li>
