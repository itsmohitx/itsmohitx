...
region: recapture-pattern        # one of the six region slugs (see content/regions.ts)
...
Follow an idea to :trail[another essay]{slug=the-business-model-is-the-risk} or a
:region[region]{slug=verification}.
```
...
The six regions, their map positions, and the annotated connections between them
